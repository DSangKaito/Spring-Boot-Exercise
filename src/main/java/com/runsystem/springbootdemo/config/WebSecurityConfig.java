...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    /** userservice from project service */
    @Autowired
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().cors() // Ngăn chặn request từ một domain khác
                .and().authorizeRequests().antMatchers("/api/login").permitAll() // Cho phép tất cả mọi người truy cập vào địa chỉ này
                .antMatchers("/api/add").permitAll().antMatchers(AUTH_WHITELIST).permitAll().anyRequest()
